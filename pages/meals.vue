<template>
  ...
  <div class="max-w-2xl mx-auto mt-24">
    <h1 class="mx-auto text-lg text-center">Track Calories</h1>
    <p class="mx-2 my-1">Total Calories: {{ totalCalories }}</p>
      ...
      <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-6 py-4">{{ m.calories }}</td>
          </tr>
        </tbody>
      </table>
    </div>
...
export default {
  ...
  mounted() {
    const uid = localStorage.getItem("track_eat_user_id");
    this.$fire.firestore
      .collection("meals")
      .doc(uid)
      .onSnapshot((doc) => {
        this.meals = doc.data().items;
      });
  },
  computed: {
    totalCalories() {
